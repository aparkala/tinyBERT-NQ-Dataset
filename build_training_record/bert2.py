...
flags.DEFINE_string("vocab_file", 'BERT_TINY/vocab-nq.txt',
...
flags.DEFINE_string("train_precomputed_file", 'train_sample_record',
                    "Precomputed tf records for training.")

flags.DEFINE_integer("train_num_precomputed", 200,
                     "Number of precomputed tf records for training.")

flags.DEFINE_string(
    "predict_file", 'nq-dev-00.jsonl.gz',
...
flags.DEFINE_string(
    "init_checkpoint", 'BERT_TINY/bert_model.ckpt',
...
vocab = {}
with open(FLAGS.vocab_file, encoding="utf8") as f:
...
def get_synonym(word):
  ...
  if syns == []:
    return vocab[randrange(30000)]
  else:
    return syns[randrange(len(syns))].lemmas()[0].name()
...
def create_example_from_jsonl(line):
  ...
  #create an augmented example
  if example['answers'][0]['candidate_id'] != -1:
    start_span = example['answers'][0]['span_start']
    end_span = example['answers'][0]['span_end']
    aug_example = copy.deepcopy(example)
    
    context_words = example['contexts'].split(' ')
    aug_context_words = copy.deepcopy(context_words)
    for idx, word in enumerate(context_words[start_span:end_span]):
        if word[0] == '[':
            continue
        else:
            if (randrange(10)<=7):
                aug_context_words[start_span+idx] = str(word)
            else:
                aug_context_words[start_span+idx] = get_synonym(word)
    aug_example['contexts'] = (' ').join(aug_context_words)
    examples = [example, aug_example]
    for i in range(0,2):
      yield examples[i]
  return example
...
def runBERT(train_rec_loc, num_train_samples, output_predict_file, to_train, to_predict):

  FLAGS.train_precomputed_file = train_rec_loc
  FLAGS.train_num_precomputed = num_train_samples
  FLAGS.do_train = to_train
  FLAGS.do_predict = to_predict
  FLAGS.output_prediction_file = output_predict_file
